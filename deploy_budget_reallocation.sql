-- Budget Reallocation System - Core Tables and Functions
-- Safe deployment without problematic triggers
...
CREATE INDEX IF NOT EXISTS idx_budget_reallocations_budget_id ON public.budget_reallocations(budget_id);
CREATE INDEX IF NOT EXISTS idx_budget_reallocations_user_id ON public.budget_reallocations(user_id);
CREATE INDEX IF NOT EXISTS idx_budget_reallocations_transaction_id ON public.budget_reallocations(transaction_id) WHERE transaction_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_budget_reallocations_created_at ON public.budget_reallocations(created_at);

-- Enable RLS
ALTER TABLE public.budget_reallocations ENABLE ROW LEVEL SECURITY;

-- RLS policies
DROP POLICY IF EXISTS "Users can manage their own budget reallocations" ON public.budget_reallocations;
...
RETURNS TABLE(
    ...
    SELECT 
        ...
        COALESCE(c.spent, 0) as total_spent,
        c.allocated,
        GREATEST(COALESCE(c.spent, 0) - c.allocated, 0) as deficit
    FROM public.categories c
    LEFT JOIN public.transactions t ON t.category_id = c.id
    WHERE c.budget_id = p_budget_id 
      AND c.user_id = auth.uid()
      AND (c.allocated = 0 OR c.allocated < COALESCE(c.spent, 0))
    GROUP BY c.id, c.name, c.allocated, c.spent
